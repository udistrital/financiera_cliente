"use strict";

/**
 * @ngdoc function
 * @name financieraClienteApp.decorator:TextTranslate
 * @description
 * # TextTranslate
 * Decorator of the financieraClienteApp
 */
var text_es = {
    A_S002: "Apropiación Actualizada",
    ABONO_CUENTA: "Abono en Cuenta",
    ACH: "ACH",
    ACTIVO: "Activo",
    ACTA:"Acta",
    ACTA_DEVOLUCION:"Acta Devolución",
    ACTUALIZADO: "Actualizado",
    ACTUALIZADO_CORRECTAMENTE: "Actualizado correctamente",
    ACTUALIZAR: "Actualizar",
    ACTUALIZAR_COMPROMISO: "Actualizar Compromiso",
    ACTUALIZAR_CONCEPTO: "Actualizar Concepto",
    ADICION_FUENTE_FINANCIAMIENTO: "Adición Fuente de Financiamiento",
    AFECTACION: "Afectación",
    AFECTACION_DE_LA_ANULACION : "Afectación de la Anulación",
    AFECTACION_CONCEPTO: "Ingresar Afectación por Conceptos",
    AFECTACION_CONTABLE: "Afectación Contable",
    AFECTACION_PRESUPUESTAL: "Afectación Presupuestal",
    AGREGAR_BANCO: "Agregar Banco",
    AGREGAR_CUENTA_BANCARIA: "Agregar Cuenta Bancaria",
    AGREGAR_DEPENDENCIA: "Agregar Dependencia",
    AGREGAR_REQUISITO: "Añadir Requisito",
    AGREGAR_REGISTRO_ESTUDIANTE: "Agregar registro de estudiante",
    AGREGAR_MAS_CUENTAS:"¿Desea agregar más cuentas?",
    AGREGAR_SUCURSAL: "Agregar Sucursal",
    ALERTA_CAMPOS_BUSQUEDA: "Por favor, indique al menos un criterio de búsqueda",
    ALERTA_COMPLETAR_DATOS: "Complete todos los campos",
    ALERTA_CREACION_TIPOCOM_CORRECTA: "Tipo de comprobante creado correctamente",
    ALERTA_CREACION_TIPOCOM_INCORRECTA: "Error al crear tipo de comprobante",
    ALERTA_CREACION_COM_CORRECTA: "Comprobante creado correctamente",
    ALERTA_CREACION_COM_INCORRECTA: "Error al crear comprobante",
    ALERTA_JUSTIFICACION_RECHAZO: "Indique una justificación por el rechazo",
    ALERTA_ERROR_RECHAZO: "Error al registrar rechazo",
    ALERTA_NO_COMPROBANTES: "No hay comprobantes para la vigencia",
    ALERTA_NO_REGISTROS: "No hay registros para este comprobante",
    ALERTA_RECHAZO_CORRECTO: "Rechazo realizado correctamente",
    ALERTA_SELECCIONAR_CUENTA: "Es necesario seleccionar la cuenta contable",
    ALERTA_SELECCIONAR_CUENTA_SIN_HIJOS: "Únicamente se pueden seleccionar cuentas que no tengan hijos",
    ALERTA_SELECCIONAR_PROVEEDOR: "Es necesario seleccionar un proveedor",
    ANADIR: "Añadir",
    ANULACION_CDP_TITULO_FORM: "Anulación de CDP",
    ANULACION_PARCIAL: "Parcial",
    ANULACION_RP: "Información de anulación de RP",
    TOTAL_ANULADO_RP: "Total Anulado Rp",
    ANULACION_TOTAL: "Total",
    ANULAR: "Anular",
    ANULAR_PRESUPUESTO: "Anular Presupuesto",
    ANO: "Año",
    ANALISIS_RIESGOS: "Análisis de Riesgos",
    APELLIDOS: "Apellidos",
    APROBACION_PRESUPUESTO: "Aprobación de Presupuesto",
    APROBACION_CONTABLE: "Aprobación de Contabilidad",
    APROBAR_AVANCE: "Aprobar Avance",
    APROBAR: "Aprobar",
    APORTANTE:"Aportante",
    ARBOL_RUBROS_HOMMOLOGADO:"Arbol Rubros Homologados",
    AVANCE_NO: "Avance No.",
    AVANCES: "Avances",
    BANCO: "Banco",
    BANCO_GIRADOR: "Banco Girador",
    BANCO_RECEPTOR: "Banco Receptor",
    BANCOS: "Bancos",
    BASE_UVT: "Base UVT",
    BENEFICIARIO: "Beneficiario",
    BORRAR: "Borrar",
    BUSCAR: "Buscar",
    CANCELACION_INVERSION: "Cancelación Inversión",
    CALENDARIO_TRIBUTARIO: "Calendario Tributario",
    CARGUE_MASIVO_RP: "Cargue Masivo Registros Presupuestales",
    CARGANDO: "Cargando",
    CARNET: "Carné",
    CAMBIAR_PADRE: "Cambiar Padre",
    CATEGORIA: "Categoría",
    CATEGORIAS_TIPOS: "Categorías y Tipos",
    CAUSAL_REINTEGRO:"Causal Reintegro",
    CDP: "CDP",
    CDP_NUMERO: "CDP No.",
    CENTRO_COSTO: "Centro de costos",
    CHEQUE: "Cheque",
    CHEQUE_INICIAL:"Número cheque inicial",
    CHEQUE_FINAL:"Número cheque final",
    CHEQUERA:"Chequera",
    CHEQUES_DISPONIBLES:"Cheques Disponibles",
    CIERRE_PERIODO_PAC:"CIERRE PERIODO",
    CIUDAD: "Ciudad",
    CLASE:"Clase",
    CLASIFICACION: "Clasificación",
    CODIGO: "Código",
    CODIGO_ABREVIACION: "Código de Abreviación",
    CODIGO_ACH: "Código ACH",
    CODIGO_CONVENIO: "Código de Convenio",
    CODIGO_COMPROBANTE: "Código del comprobante",
    CODIGO_ESTUDIANTE: "Código del estudiante",
    CODIGO_PADRE: "Código Padre",
    CODIGO_PROYECTO: "Código del Proyecto",
    CODIGO_SUPER: "Código Superintendencia",
    CUENTAS_ASOCIADAS:"Cuentas Asociadas",
    MSN_DEBE_FORMA_VIGENCIA: "El formato para la vigencia es AAAA",
    CAMPOS_OBLIGATORIOS:"Debe diligenciar los campos marcados",
    CAMPOS_OBLIGATORIOS_SOLICITANTE:"Debe diligenciar los campos del solicitante",
    CAMPOS_OBLIGATORIOS_BENEFICIARIO:"Debe diligenciar los campos del beneficiario",
    CAMPOS_OBLIGATORIOS_AVANCE:"Debe diligenciar todos los campos para agregar un avance",
    CODIGO_BANCO: "Código del Banco",
    COMPANIA: "Compañía",
    COMPROMISO: "Compromiso",
    COMPROMISO_NO: "Compromiso No.",
    COMPROMISOS: "Compromisos",
    CONCEPTO: "Concepto",
    CONCEPTOS: "Conceptos",
    CONCEPTOS_SELECCIONADOS: "Conceptos Seleccionados",
    CONFIGURAR: "Configurar",
    CONFIRMACION_ACTIVO_TIPO_COMPROBANTE: "¿Desea activar el tipo de comprobante",
    CONFIRMACION_INACTIVO_TIPO_COMPROBANTE: "¿Desea inactivar el tipo de comprobante",
    CONFIRMACION_EDICION_TIPO_COMPROBANTE: "¿Desea editar el tipo de comprobante",
    CONFIRMACION_ANULACION_OP: "¿Está seguro de querer anular la Orden de pago ",
    CONSECUTIVO: "Consecutivo",
    CONSULTA_APROBACION_INICIAL: "Consulta de Apropiación Inicial",
    CONSULTA_INGRESOS: "Consulta Ingresos",
    CONSULTA_RUBRO: "Consulta de Rubros",
    CONSULTA_RUBRO_HOMOLOGADO: "Consulta de Rubros Homologados",
    CONTRATO: "Contrato",
    CONTROL_SALDOS_INICIALES: "Control de Saldos Iniciales de Bancos",
    CONVENIO: "Convenio",
    CONSULTA_RELACIONES_DEVOL:"Consulta Relaciones de Devolución",
    CONSULTA_INGRESOS_SIN_SF:"Consulta Ingresos sin Situación de Fondos",
    CORREO_ELECTRONICO: "Correo Electrónico",
    COMPROBACION_APROBACION_INICIAL: "Comprobación de apropiación inicial",
    CONSULTA_INVERSIONES:"Consulta Inversiones",
    CONSULTA_CANCELACION_INVERSIONES:'Consulta Cancelación Inversiones',
    COMPRADOR:"Comprador",
    CREACION: "Creación",
    CREACION_ACTA_COMPRA:"Creación Acta Compra",
    CREACION_COMPROBANTE: "Creación de comprobante",
    CREACION_CHEQUE: "Creación Cheque",
    CREACION_TIPO_COMPROBANTE: "Creación de tipos de comprobante",
    CREADO: "Creado",
    CREAR_FUENTE: "Crear Fuente Financiamiento",
    CREDITO: "Crédito",
    CRP: "CRP",
    CUENTA: "Cuenta",
    CUENTA_BANCARIA: "Cuenta Bancaria",
    CUENTAS_BANCARIAS: "Cuentas bancarias",
    CUENTAS_CONTABLES: "Cuentas Contables",
    CUENTA_CONTABLE: "Cuenta contable",
    CUENTA_CREDITO: "Cuenta Crédito",
    CUENTA_CONTRA_CREDITO: "Cuenta Contra Crédito",
    CUENTA_GIRO: "Cuenta Giro",
    CUENTA_RECEPTORA:"Cuenta Receptora",
    CUENTAS_SELECCIONADAS: "Cuentas Seleccionadas",
    CUENTA_PADRE: "Cuenta Padre",
    DATOS_BASICOS: "Datos Básicos",
    DATOS_CANCELACION: "Datos Cancelación",
    DATOS_NECESIDAD: "Datos de la Necesidad",
    DATOS_MOVIMIENTO: "Datos del Movimiento Presupuestal",
    DATOS_SOLICITUD_MODIFICACION_PRES: "Datos de la Solicitud",
    DATOS_REG_INGRESO: "Datos para este Ingreso",
    DE: "de",
    DEBITO: "Débito",
    DEDUCIBLE: "Deducible",
    DEFINITIVA: "Definitiva",
    DENOMINACION: "Denominación",
    DEPARTAMENTO: "Departamento",
    DEPENDENCIA: "Dependencia",
    DEPENDENCIAS: "Dependecias",
    DEPENDENCIA_DESTINO: "Dependencia Destino",
    DEPENDENCIA_SOLICITANTE: "Dependencia Solicitante",
    DESCRIPCION: "Descripción",
    DESCRIPCION_AVANCES: "Descripción de Avances",
    DESCRIPCION_COMPROMISO: "Descripción Compromiso",
    DESCRIPCION_PLAN: " Descripción del nuevo plan...",
    DESAPROBACION_CONTABLE: "Desaprobación Contable",
    DESASOCIAR_CONCEPTO: "El concepto y las cuentas contables asociadas serán eliminados",
    DESAPROBACION_PRESUPUESTAL: "Desaprobación Presupuestal",
    DESEA_ACTUALIZAR_COMPROMISO: "¿Desea actualizar el Compromiso?",
    DESEA_ACTUALIZAR_DESCUENTO: "¿Desea actualizar el Descuento o Impuesto?",
    DESEA_ACTUALIZAR_CONCEPTO: "¿Desea actualizar el Concepto?",
    DESEA_ACTUALIZAR_CUENTA: "¿Desea actualizar la cuenta contable?",
    DESEA_CREAR_CATEGORIA: "¿Desea crear una nueva categoría?",
    DESEA_CREAR_COMPROMISO: "¿Desea crear un nuevo compromiso?",
    DESEA_CREAR_TIPO_COMPROMISO: "¿Desea crear un nuevo tipo de compromiso?",
    DESEA_CREAR_CONCEPTO: "¿Desea crear un nuevo concepto tesoral?",
    DESEA_CANCELAR_COMPROMISO: "¿Desea cancelar el Compromiso?",
    DESEA_CREAR_CUENTA: "¿Desea crear nueva cuenta contable?",
    DETALLE: "Detalle",
    DETALLE_AVANCES: "Detalle de Avances",
    DETALLE_CRP: "Detalle CRP",
    DETALLE_COMPROBANTE: "Detalle del comprobante",
    DETALLE_OP: "Detalle Orden de Pago",
    DETALLE_ORDEN_PAGO: "Detalle Orden de Pago",
    DETALLE_PRODUCTO: "Detalle del Producto",
    DETALLE_PROVEEDOR: "Detalle Proveedor",
    DETALLE_RP: "Registro Presupuestal No ",
    DETALLE_UNIDAD_EJECUTORA: "Detalle Unidad Ejecutora",
    DETALLE_VALOR_PAGO: "Detalle Valor Pago",
    DETALLE_TIPO_TRANSACCION: "Detalle Tipo Transacción",
    DETALLE_VERSION:"Detalle Versión",
    DEVOLUCION:"Devolución",
    DEVOLUCION_TRIBUTARIA:"Devolución Tributaria",
    DIFERENCIA: "Diferencia",
    DIRECCION: "Dirección",
    DISPONIBLE: "Disponible",
    DIAS_VENCIMIENTO:"Días de vencimiento",
    DOCUMENTO: "Documento",
    DOCUMENTO_NIT: "Documento o Nit.",
    DURACION: "Duración",
    EDICION_TIPO_COMPROBANTE: "Edición de comprobante",
    EDITAR: "Editar",
    EDITAR_CUENTA_BANCARIA: "Editar Cuenta Bancaria",
    EDITAR_PLAN_CUENTAS: "Editar plan de Cuentas",
    EDITAR_REQUISITO_AVANCE: "Editar Requisito de Avance",
    EDITAR_SUCURSAL: "Editar sucursal",
    EDITAR_TIPO_AVANCE: "Editar Tipo de Avance",
    EDITAR_CUENTA: "Editar Cuenta Contable",
    EFECTIVO: "Efectivo",
    ELEGIR: "Elegir",
    EGRESO: "Egreso",
    ELABORADO_POR: "Elaborado Por",
    ELIMINADO: "Eliminado",
    ELIMINARA: "Se eliminará",
    CANCELAR_COMPROMISO: "Cancelar Compromiso",
    EDITAR_REGISTRO_ESTUDIANTE: "Editar registro de estudiante",
    ENTIDAD: "Entidad",
    EMISOR:"Emisor",
    ENDOSAR_A_TERCERO: "Endosar a tercero",
    ESTUDIANTE_NO_ENCONTRADO:"Estudiante no encontrado",
    ENTRADA_ALMACEN: "Entrada de Almacén",
    ERROR_ELIMINAR: "Error al eliminar",
    ERROR_ACTUALIZAR: "Error al actualizar",
    ERROR_LLENAR: "Debe llenar todos los campos",
    ERROR_OBSERVACIONES: "Debe llenar todas las observaciones",
    ERROR_REQUISITOS: "Debe seleccionar todos los requisitos",
    ESTADO: "Estado",
    ETAPA: "Etapa",
    E_0458: "Los datos enviados son erróneos",
    E_0459: "No se pudo registrar la información",
    E_0460: "No se pudo Eliminar",
    E_0461: "No se Puede Obtener La Fecha Actual.",
    E_04566: "No es posible modificar el registro debido a que este ha sido usado en otras transacciones",
    E_0471: "No es posible realizar actualización debido al estado del registro",
    E_0462: "Error al cargar los datos.",
    E_22001: "La longitud de uno de los valores ingresados excede al máximo permitido",
    E_23502: "No se puede realizar la operación Solicitada",
    E_23503: "No se puede realizar la operación Solicitada",
    E_23505: "Ya existe un registro con uno de los parámetros únicos ingresados",
    E_25P02: "Operación No Permitida. ",
    E_901: "No se ha podido validar los requisitos",
    E_A02: "Debe digitar el motivo de la anulación",
    E_A03: "Debe seleccionar el tipo de anulación a realizar",
    E_A04: "Debe digitar el valor de la anulación",
    E_A05: "Debe seleccionar el rubro afectado por el CDP a anular.",
    E_A06: "Debe seleccionar el rubro afectado por el RP a anular.",
    E_A07: "El valor debe ser mayor a 0",
    E_A08:  "La necesidad no se encuentra aprobada",
    E_ISF001: "Rubro seleccionado no valido",
    E_ISF002: "Error al intentar registrar Ingreso",
    E_CDP001: "Expedición de CDP no aprobada, algunos valores superan el saldo de las Apropiaciones.",
    E_CDP002: "No se pudo cargar los rubros de la solicitud",
    E_CI001:"Existe cancelación creada para la inversion",
    E_COMP1: "Elija una categoría",
    E_COMP2: "Elija un tipo de compromiso",
    E_GS001:"Error al actualizar ciudad",
    E_GS002:"Error al actualizar pais",
    E_GS003:"Error al actualizar dirección",
    E_GS004:"Error al actualizar nombre sucursal",
    E_GS005:"Error al actualizar telefono",
    E_GS006:"Error al actualizar departamento",
    E_GS007:"Error desvinculando sucursales",
    E_GS008:"Error Vinculando sucursales",
    E_EDO001: "Ingrese el valor del endoso que no sobrepase el valor máximo",
    E_LA0001:"El valor a legalizar es mayor que el valor del avance",
    E_LT0001:"Existe una version para las fechas establecidas",
    E_MODP001: "Debe seleccionar los rubros y/o llenar los datos requeridos para esta solicitud.",
    E_MODP002: "Todos los campos son obligatorios para agregar el rubro a la solicitud",
    E_MODP003: "El rubro destino del movimiento no tiene apropiacion para la vigerncia actual.",
    E_MODP004: "El saldo del rubro origen del movimiento no cubre el valor del movimineto",
    E_MODP005: "No se pudo obtener el saldo de la apropiación origen del movimiento",
    E_MODP006 :"No se Pudo registrar una de las disponibilidades para este movimiento",
    E_A12: "Valor a anular supera el saldo de la disponibilidad en el rubro seleccionado",
    E_MODP007: "No se pudo aprobar la solicitud de Modificación Presupuestal",
    E_MODP008: "El Arbol De Las Apropiaciones No Esta Balanceado Luego De Realizar Los Movimientos.",
    E_MODP009: "No se pueden Cargar los tipos de movimiento.",
    E_MODP010: "No se puede comprobar el balanceo del arbol.",
    E_MODP011: "No se pudo registrar el movimiento.",
    // op masivas
    E_OPM_0001: "No se encontraron ordenes de pago relacionadas a la liquidacion",
    E_HO_CONC_TITAN: "Error al obtener conceptos de Titán",
    E_OPN_01_1: "Error en la estructura de datos para Crear a OP de Nomina. ",
    E_OPN_01_2: "No se obtuvo Id de la liquidación. ",
    E_OPN_01_3: "No se obtuvo información del detalle de la liquidación del modulo Titán. ",
    E_OPN_01_4: "No se se cuenta con un detalle de la liquidación del modulo Titán. ",
    E_OPN_01_5: "Error en la comunicación con los servicios de Kronos. ",
    E_OPN_02: "Error al intentar registrar la Orden de pago. ",
    E_OPN_02_1: "Error al obtener Valor Calculado del detalle de la liquidación. ",
    E_OPN_02_2: "Error al obtener el Id del Concepto de Titán en detalle de la liquidación. ",
    E_OPN_02_3: "Id Concepto Titán no cuenta con concepto Homologado en Kronos. ",
    E_OPN_02_4: "Error al intentar registrar los conceptos de orden de pago. ",
    E_OPN_02_5: "Error al intentar registrar Movimientos Contables. ",
    E_OPP_01: "Error al intentar registrar la Orden de pago. ",
    E_OPGC_01:"Orden de pago no valida",
    E_VBO001: "Ingrese el Valor Base de Retención para estimar el descuento",
    E_HOCO_01: "Error al intentar registrar la Homologación de concepto. ",
    E_HOCO_02: "Ya existe la Homologación. ",
    S_HOCO_01: "Homologación de Concepto registrada exitosamente. ",
    E_UPDATE_HOCO_01: "Error al intentar actualizar la Homologación de concepto. ",
    S_UPDATE_HOCO_01: "Homologación de concepto actualizada exitosamente. ",
    E_OPP_02: "Error al intentar registrar la afectación por conceptos. ",
    E_OPP_03: "Error al intentar registrar los Movimientos Contables. ",
    E_OPP_UPD_01: "Error al intentar Actualizar los campos de la Orden de pago. ",
    E_OPP_UPD_02: "Error al intentar Actualizar los campos antiguos de conceptos relacionados a la Orden de pago. ",
    E_OPP_UPD_03: "Error al intentar Actualizar los campos antiguos de movimientos contables relacionados a la Orden de pago. ",
    E_OPP_UPD_04: "Error al intentar Actualizar los campos de conceptos relacionados a la Orden de pago. ",
    E_OPP_UPD_05: "Error al intentar Actualizar los campos de movimientos contables relacionados a la Orden de pago. ",
    E_RB001: "Debe Asignar Un Código Al Rubro",
    E_RB002: "Debe Asignar Una Descripción al Rubro",
    E_RB003: "Debe seleccionar un rubro",
    E_RB005: "Debe Seleccionar El o Los Productos Asociados Al Rubro",
    E_RB006: "La Distribución Del Rubro En Los Productos No Es igual Al 100%",
    E_RB007: "No se puede obtener el saldo de los Rubros para comparar.",
    E_RB008: "No se puede Registrar el rubro , tal vez ya existe un registro con ese código o el rubro Padre tiene apropiaciones registradas.",
    E_RP001: "El valor solicitado supera el saldo del RP en el rubro solicitado",
    E_RP002: "No se pudo cambiar el estado de la solicitud",
    E_AP001: "Este rubro no tiene apropiación asignada para la vigencia seleccionada.",
    E_AP002: "La Apropiación de este rubro ya ha sido aprobada y no se puede modificar.",
    E_AP003: "Las sumas de los rubros a aprobar no son iguales",
    E_AP004: "Plan De Cuentas Presupuestales No Aprobado",
    E_C92011: "Un concepto o clasificador no puede ser creado dentro de un concepto, únicamente dentro de un clasificador diferente de si mismo",
    E_TRANS_01: "Error en la estructura de parámetros de entrada",
    E_OP_E_ESTADO: "Error al intentar registrar estado de orden de pago",
    E_SI1: "Error al asignar saldo inicial",
    ESTAMPILLA_UD: "Estampilla Universidad Distrital",
    ESTAMPILLA_PROCULTURA: "Estampilla Procultura",
    ESTAMPILLA_PRO_ADULTO_MAYOR: "Estampilla Adulto Mayor",
    EXTRAORDINARIO:"Extraordinario",
    EXPIDIO: "Expidió",
    FACTURA: "Factura",
    FACULTAD: "Facultad",
    FECHA: "Fecha",
    FECHA_CANCELACION: "Fecha Cancelación",
    FECHA_COMPRA_EVENTO: "Fecha de compra o evento",
    FECHA_COMPRA: "Fecha Compra",
    FECHA_CONSIG: "Fecha de Consignación",
    FECHA_CREACION: "Fecha Creación",
    FECHA_DOCUMENTO: "Fecha del Documento",
    FECHA_FIN: "Fecha final",
    FECHA_FINAL: "Fecha Final",
    FECHA_INICIO: "Fecha Inicio",
    FECHA_CIERRE:"Fecha Cierre",
    FECHA_OFICIO: "Fecha del Oficio",
    FECHA_REGISTRO: "Fecha de Registro",
    FECHA_INGRESO: "Fecha Ingreso",
    FECHA_CONSIGNACION: "Fecha de consignación",
    FECHA_EXPIRACION: "Fecha de Expiración",
    FILTRO_NOTIFICACION: "Filtrar notificación",
    FECHA_MOVIMIENTO: "Fecha Movimiento",
    FECHA_SOLICITUD: "Fecha de solicitud:",
    FECHA_VENCIMIENTO:"Fecha Vencimiento",
    FECHA_REDENCION:"Fecha Redención",
    FECHA_EMISION:"Fecha Emisión",
    FECHA_VENTA:"Fecha Venta",
    FECHA_PACTO:"Fecha Pacto",
    FUENTE:"Fuente",
    FINALIZAR: "Finalizar",
    FINANCIERO: "Financiero",
    FINANCIACION: "Financiación",
    FORMA_PAGO: "Forma Pago",
    FORMA_INGRESO: "Forma de Ingreso",
    FORMULARIO_FUENTE: "Formulario Creación Fuente de Financiamiento",
    FUENTES_AFECTADAS: "Fuentes afectadas",
    FUENTES_FINANCIACION: "Fuentes Financiación",
    FUENTES_FINANCIAMIENTO: "Fuentes de Financiamiento",
    FUENTE_FINANCIACION_CODIGO: "Código Fuente",
    FUENTE_FINANCIAMIENTO: "Fuente de Financiamiento",
    FUENTE_FINANCIAMIENTO_DESTINO: "Fuente de Financiamiento de Destino",
    FUENTE_FINANCIAMIENTO_EXISTE: "La Fuente de Financiamiento ya existe",
    FUENTE_FINANCIAMIENTO_ORIGEN: "Fuente de Financiamiento de Origen",
    FUE_ACTUALIZADO: "ha sido actualizado",
    FUE_ELIMINADO: "ha sido eliminado",
    FUNCIONAMIENTO: "Funcionamiento",
    GENERACION_PAC: "Generar Reporte PAC",
    GESTIONAR_CUENTAS_BANCARIAS: "Gestionar cuentas bancarias",
    GESTIONAR_SUCURSALES: "Gestionar sucursales",
    GIRO: "Giro",
    GIROS: "Giros",
    GUARDAR: "Guardar",
    HOMOLOGACION: "Homologación",
    IMPUESTOS: "Impuestos",
    IMPUESTOS_DESCUENTOS: "Impuestos y Descuentos",
    INACTIVO: "Inactivo",
    IDENTIFICACION: "Identificación",
    INFORMACION_ADICIONAL_BANCO: "Información Adicional del Banco",
    INFORMACION_DOCUMENTO_ING: "Información del Documento Ingreso",
    INFORMACION_DINERO: "Información de valores",
    INFORMACION_GENERAL: "Información general",
    INFORMACION_FECHAS : "Información de fechas",
    INFORMACION_LEGALIZACIONES:"Información legalizaciones",
    INFORMACION_TERCERO: "Información de Terceros",
    INFORMACION_RELACION: "Información Relación de Devolución",
    INFORMACION_DEVOLUCION:"Información Devolución",
    INFORMACION_REG_CORRECTO: "Información registrada correctamente",
    INFORMACION_TIPO_TRANSACCION:"Información Tipo Transacción",
    INFORMACION_TRASLADO: "Información de Traslado",
    INFORMACION_REINTEGRO: "Información Reintegro",
    INFORMACION_REGISTRO_REINTEGRO: "Información Registro Reintegro",
    INFORMACION_PRESUPUESTAL: "Información presupuestal",
    INFORMACION_CHEQUERA: "Información chequera",
    INFORMACION_CHEQUE: "Información Cheque",
    INGRESAR_MONTO: "Ingresar Monto",
    INGRESAR_VALOR_FUENTE: "Ingresar Valor Total Fuente de Financiamiento",
    INGRESE_CODIGO: "Ingrese el Código",
    INGRESE_DEPENDENCIA: "Ingrese la Dependencia",
    INGRESE_DESCRIPCION: "Ingrese la Descripción",
    INGRESE_OBJETO_COMPROMISO: "Ingrese el Objeto del compromiso aquí ...",
    INGRESE_FECHA_DOCUMENTO: "Ingrese Fecha del Documento",
    INGRESE_FECHA_REGISTRO: "Ingrese Fecha de Registro",
    INGRESE_NOMBRE: "Ingrese el Nombre",
    INGRESE_NOMBRE_PLAN_CUENTAS: "Ingresa el nombre del plan de cuentas",
    INGRESE_NO_DOCUMENTO: "Ingrese el No. de Documento",
    INGRESE_SIGLA: "Ingrese la Sigla",
    INGRESE_TIPO_DOCUMENTO: "Ingrese el Tipo de Documento",
    INGRESE_VALOR_ADICION: "Ingrese el Valor Total de la Adición",
    INGRESE_VALOR_DEPENDENCIA: "Ingrese el Valor de la Dependencia",
    INGRESE_VALOR_TOTAL: "Ingrese el Valor Total",
    INGRESE_VALOR_TRASLADO: "Ingrese el Valor Total del Traslado",
    INGRESO: "Ingreso",
    INGRESO_NO: "Ingreso No.",
    INGRESOS_SIN_SF:"Ingresos sin Situación de Fondos",
    INICIAL: "Inicial",
    INGRESOS_EGRESOS_CUMPLE: "Las sumas de comprobación son iguales",
    INGRESOS_EGRESOS_NO_CUMPLE: "los valores de comprobación son diferentes",
    INVERSION:"Inversión",
    INVERSIONES:"Inversiones",
    ITEM_NO_PUEDE_SELECCIONAR: "El item no puede ser seleccionado",
    JEFE_DEPENDENCIA: "Jefe de la Dependencia",
    JUSTIFICACION: "Justificación",
    JUSTIFICACION_CONTRATO: "Justificación de Contrato",
    LEGALIZACION: "Legalización",
    LIMPIAR: "Limpiar",
    LIQUIDACION: "Liquidación",
    LEGALIZACION_AVANCE: "Legalización de avances",
    LEGALIZACION_EVENTO_COMPRA: "Legalización por evento o compra",
    LEGALIZACION_PRACTICA_ACADEMICA: "Legalización por práctica académica",
    LISTA_AVANCES: "Listado de Tipos de Avances",
    LISTA_CDP: "Lista De Certificados de Disponibilidad Presupuestal",
    LISTA_CDP_ANULACION: "Lista De Certificados De Disponibilidad Presupuestal a Anular",
    LISTA_COMPROBANTES: "Lista de Comprobantes",
    LISTA_REQUISITOS_AVANCE: "Listado de Requisitos de Avance",
    LISTA_PRODUCTOS: "Listado de Productos",
    LISTA_PRODUCTOS_ASOCIADOS: "Lista Productos Asociados Al Rubro",
    LISTA_RESERVAS: "Lista de Reservas",
    LISTA_RP: "Lista de Registros Presupuestales",
    LISTA_RP_ANULACION: "Lista de Solicitudes de Anulación de Registros Presupuestales ",
    LISTA_SOLICITUD_ANULACION_CDP: "Lista de Solicitudes de Anulación de Disponibilidad",
    LISTA_SOLICITUDES_MODIFICACION_PRESUPUESTAL: "Lista de Solicitudes de Movimientos Presupuestales",
    LISTA_TITULOS:"Lista de Títulos",
    LISTA_ACTAS:"Lista de Actas",
    LISTA_RAZON_DEVOLUCION:"LIsta Razones de Devolución",
    LUGAR_DOCUMENTO: "Lugar de Expedición",
    MATRICULA: "Matricula",
    MENSAJE_INICIAL: "Ahora puede comenzar con el desarrollo ...",
    MARCO_LEGAL: "Marco Legal",
    MES: "Mes",
    MES_ABRIL: "Abril",
    MES_AGOSTO: "Agosto",
    MES_DICIEMBRE: "Diciembre",
    MES_ENERO: "Enero",
    MES_FEBRERO: "Febrero",
    MES_JULIO: "Julio",
    MES_JUNION: "Junio",
    MES_MARZO: "Marzo",
    MES_MAYO: "Mayo",
    MES_NOBIEMBRE: "Noviembre",
    MES_OCTUBRE: "Octubre",
    MES_SEPTIEMBRE: "Septiembre",
    MODALIDAD_DE_SELECCION: "Modalidad de Selección",
    MODIFICACIONES_PRESUPUESTALES: "Modificaciones Presupuestales",
    MODIFICACION_FUENTES_FINANCIAMIENTO: "Modificación Fuentes de Financiamiento",
    MODIFICACION_PRESUPUESTAL: "Modificación Presupuestal",
    MODO_PAGO: "Modo de pago",
    MONTO_MAYOR_ADICION: "El Monto Ingresado No Corresponde al Valor de la Adición",
    MONTO_MAYOR_FUENTE_FINANCIAMIENTO: "El Monto Ingresado No Corresponde al Valor de la Fuente de Financiamiento",
    MONTO_MAYOR_TRASLADO: "El Monto Ingresado Supera el Valor de la Dependencia",
    MOTIVO_ANULACION_CDP: "Motivo de la Anulación",
    MOTIVO_RECHAZO: "Motivo del rechazo",
    MOVIMIENTO: "Movimiento",
    MOVIMIENTOS: "Movimientos",
    MSN_CAMPO_REQUERIDO: "Campo requerido*",
    MSN_TAMANO_MAX: "Sobrepasa el tamaño máximo",
    MSN_TAMANO_MIN: "Es menor al tamaño mínimo",
    MSN_DEBE_ASOCIAR_CUENTAS: "Debe asociar cuentas a la devolución ",
    MSN_DEBE_CHEQUERA:"Debe seleccionar una chequera",
    MSN_DEBE_CONCEPTO: "Debe seleccionar un concepto",
    MSN_DEBE_OP_GIRO: "Debe seleccionar ordenes de pago",
    MSN_DEBE_NOMINA: "Debe seleccionar por lo mínimo una nomina",
    MSN_DEBE_FORMA_PAGO_OP: "Debe seleccionar la forma de pago en la sección detalle orden pago",
    MSN_DEBE_IVA: "Debe indicar el valor del iva en la sección detalle orden pago",
    MSN_DEBE_LIQUIDACION: "Debe seleccionar tipo nomina, mes y año en la sección liquidación",
    MSN_DEBE_MES_SS: "Debe seleccionar el mes de la seguridad social",
    MSN_DEBE_MIN_CONCEPTO: "Debe afectar por lo menos un concepto",
    MSN_DEBE_PROVEEDOR: "Debe seleccionar el proveedor para la orden de pago",
    MSN_DEBE_REGISTRO: "Debe seleccionar el registro presupuestal",
    MSN_DEBE_ENTRADA_ALMACEN: "Debe seleccionar la entrada de almacen",
    MSN_DEBE_TIPO_OP: "Debe seleccionar el tipo de orden de pago en la sección detalle orden pago",
    MSN_DEBE_DOCUMENTO: "Debe seleccionar el tipo de documento en la sección detalle orden pago",
    MSN_DEBE_TERCERO: "Debe seleccionar un tercero",
    MSN_DEBE_TOTAL_AFECTACION: "El valor total de la afectación es distinto al valor de la orden de pago",
    MSN_DEBE_UNIDAD: "Debe seleccionar la unidad ejecutora",
    MSN_DEBE_VAL_BASE: "Debe indicar el valor bruto en la sección detalle orden pago",
    MSN_SUPERA_SALDO: "supera el saldo disponible",
    MSN_SOL_NO_ENC: "Solicitante no encontrado",
    MSN_BEN_NO_ENC: "Beneficiario no encontrado",
    MSN_RESP_NO_ENC: "Responsable no encontrado",
    MSN_TOTAL_AECTACION: "El total de la afectación al rubro",
    MSN_TIPO_AVANCE: "Elegir al menos un tipo de avance",
    MSN_OP_PLANTA_DEBE_NECESIDAD: "Debe seleccionar una necesidad",
    MSN_OP_PLANTA_DEBE_NECESIDAD_CON_RP: "Debe seleccionar una necesidad con RP asociado",
    MSN_OP_PLANTA_DEBE_LIQUIDACION_A_NECESIDAD: "No existe liquidación Asociada a la Necesidad",
    MSN_OPP_S_HO_CONC: "Homologación de conceptos exitosa",
    MSN_Y: "y",
    NATURALEZA: "Naturaleza",
    NECESIDAD: "Necesidad",
    NECESIDAD_NO: "Necesidad No. ",
    NECESIDAD_NO_APROBADA: "Necesidad Aprobada No.",
    NECESIDAD_FECHA_RECHAZO: "Fecha de rechazo",
    NECESIDAD_JUSTIFICACION_RECHAZO: "Justificación de rechazo",
    NECESIDAD_JUSTIFICACION_DESCRIPCION_RECHAZO: "Justificación",
    NO: "No. ",
    NOMBRE: "Nombre",
    NOMBRES: "Nombres",
    NOMBRE_COMPROBANTE: "Nombre del comprobante",
    NOMBRE_CONVENIO: "Nombre del Convenio",
    NOMBRE_PROYECTO: "Nombre del Proyecto",
    NOMINA: "Nómina",
    NOMINAL:"Nominal",
    NOTA_DEVITO: "Nota Débito",
    NOTIFICACION_PENDIENTE: "Notificaciones Pendientes",
    NOTIFICACION_VISTA: "Notificaciones Vistas",
    NO_APLICA: "No aplica",
    NO_CDP: "No. CDP",
    NO_CRP: "No. CRP",
    NO_DIAS: "No. de días",
    NO_DATA: "No hay datos disponibles",
    NO_DOCUMENTO: "No. Documento",
    NO_ENCONTRADO: "No encontrado",
    NO_FACTURA: "No. de factura",
    NO_ITEMS: "Número de items",
    NO_REQUISITOS_REGISTRADOS: "No hay requisitos registrados para este tipo de avance",
    NO_OFICIO: "Oficio No",
    NO_QUINCENA: "Nro. Quincena",
    NO_CHEQUE:"No. Cheque",
    NO_ENCONTRADO_RESULTADOS: "No se han encontrado resultados para el",
    NUEVA_CUENTA: "Nueva Cuenta",
    NUEVA_CATEGORIA: "Nueva Categoría",
    NUEVO: "Nuevo",
    NUEVA: "Nueva",
    NUEVO_CONCEPTO: "Nuevo Concepto",
    NUEVO_OP_PROVEEDOR: "Nuevo OP Proveedor",
    NUM_CHEQUES:"Número Cheques",
    HORA_CATEDRA: "Hora Cátedra",
    NUEVO_OP_SEGURIDAD_SOCIAL: "Nuevo OP Seguridad Social",
    NUEVO_PLAN_CUENTAS: "Crear Nuevo Plan de cuentas",
    NUEVO_REQUISITO_AVANCE: "Nuevo Requisito de Avance",
    NUEVO_TIPO_AVANCE: "Nuevo Tipo de Avance",
    NUEVO_COMPROMISO: "Nuevo Compromiso",
    NUEVO_TIPO_COMPROMISO: "Nuevo Tipo Compromiso",
    NUMERO: "Número",
    NUMERO_CELULAR: "Número Celular",
    NUMERO_CUENTA:"Número de cuenta",
    NUMERO_ELABORACION: "No. de Elaboración",
    NUMERO_SELECCION: "No. de Selección",
    NUMERO_TELEFONICO: "Número telefónico",
    NUMERO_TRASLADO: "Número de Traslado",
    NUMERO_OPERACION:"Número Operación",
    OBJETIVO: "Objetivo",
    OBJETO: "Objeto",
    OBJETO_CONTRACTUAL: "Objeto Contractual ",
    OBSERVACIONES: "Observaciones",
    OFICIO: "Oficio No.",
    OP_TITULO: "Ordenes de Pago",
    OPERACION: "Operación",
    OPERAR_CONCEPTO: "Operar Concepto",
    ORDENADOR_GASTO: "Ordenador del Gasto",
    ORDEN_PAGO: "Ordenes de Pago",
    ORDEN_DE_PAGO: "Orden de Pago",
    ORDEN_PAGO_ACTUALIZAR: "Actualizar Ordenes de Pago",
    ORDEN_PAGO_SELECCIOANR: "Seleccionar Orden de Pago",
    ORDEN_DEVOLUCION: "Orden de Devolución",
    ORIGEN: "Origen",
    OPCIONES: "Opciones",
    ORDINARIO: "Ordinario",
    OPM_E001: "Conceptos Homologados no coinciden con los de los rubros de RP",
    OPM_E002: "Saldo del RP es menor al valor solicitado en la liquidación",
    OPM_E003: "No se encontró RP asociado al contrato liquidado",
    OPM_E004: "No se encontró homologación de conceptos",
    PAGO_REPORTADO: "Pago reportado",
    PACTO_VENTA: "Pacto de Venta",
    PAGOS_REALIZADOS: "Pagos Realizados",
    PAGO_MATRICULA: "Pago Matricula",
    PAGO_SEGURO: "Pago Seguro",
    PAGO_CARNET: "Pago Carné",
    PAIS: "País",
    PARA: "Para",
    PERIODO:"Periodo",
    PLANES_CUENTAS: "Planes de Cuentas",
    PLANTA: "Planta",
    PLAN_CUENTAS: "Plan de Cuentas",
    PLAN_CUENTAS_MAESTRO: "Plan de Cuentas Maestro",
    PORCENTAJE: "Porcentaje",
    POR_IVA: "% IVA",
    PENDIENTE_NECESIDAD: "Pendiente por Necesidad",
    PRIMERA: "Primera",
    PRINCIPIO_PARTIDA_DOBLE_ADVERTENCIA: "¡Recuerda que la suma de los montos debitados y de los montos acreditados debe ser la misma e igual al valor dado sobre el concepto!",
    PRINCIPIO_PARTIDA_DOBLE_CUMPLE: "¡El principio de la partida Doble o dualidad se cumple!",
    PROCESO_COMPLETADO: "Proceso Completado",
    PRODUCTOS: "Productos",
    PRODUCTOS_SELECCIONADOS: "Productos Seleccionados",
    PROVEEDOR: "Proveedor",
    PROYECTO_CURRICULAR: "Proyecto Curricular",
    PROYECTO_INVESTIGACION: "Proyecto de Investigación",
    QUITAR: "Quitar",
    QUITAR_FUENTE: "Quitar Fuente",
    QUITAR_RUBRO: "Quitar Rubro",
    RECHAZAR: "Rechazar",
    RAZON_DEVOLUCION:"Motivo de Devolución",
    RECIBO: "Recibo",
    REDONDEO: "Redondeo",
    REFERENCIA: "Referencia",
    REGISTRAR_PRODUCTO: "Registrar Producto",
    REGISTRO_TITULO:"Registrar Titulo",
    REGISTRO_CORRECTO: "Se Registro Correctamente",
    REGISTRO_INGRESOS: "Registro de Ingresos",
    REGISTRO_PRESUPUESTAL: "Certificado Registro Presupuestal",
    REGISTRO_PRESUPUESTAL_NO: "RP No.",
    REGISTRO_RUBRO: "Registro de Rubro",
    REGISTRO_SOLICITUD_MODIFICACION_PRES: "Solicitud de Modificación Presupuestal",
    REFERENCIA_PAGO: "Referencia de pago",
    REINTEGROS:"Reintegros",
    REPORTE_PAC:"REPORTE INGRESOS Y EGRESOS",
    REPORTE_PAC_TOTALES:"REPORTE INGRESOS Y EGRESOS TOTALES POR FUENTE",
    REQUISITOS: "Requisitos",
    REQUISITOS_AVANCE: "Requisitos de Avance",
    RESPONSABLE: "Responsable",
    RESPONSABLE_SELECCION_PRESUPUESTO: "Responsable Sección De Presupuesto",
    RESUMEN_AFECTACION_PRESUPUESTAL: "Resumen Afectación Presupuestal",
    RELACION_DEVOLUCION: "Relación Devolución",
    RESUMEN: "Resumen",
    RUBRO: "Rubro",
    RUBROS: "Rubros",
    ARBOL_HOMOLOGADO:"Arbol de Rubros Homologado",
    RUBROS_AFECTADOS_MOVIMIENTO: "Rubros afectados por este Movimiento",
    RUBROS_ASIGNADOS_FUENTE: "Rubros Asignados Fuente de Financiamiento",
    RUBRO_DESTINO: "Rubro de Destino",
    RUBRO_MAYOR_APROPIACION: "Uno de los Rubros Ingresados Supera el Valor de la Apropiación Inicial",
    RUBRO_ORIGEN: "Rubro de Origen",
    RUBRO_SELECCIONADO: "Rubros Seleccionado",
    RUBROS_SOLICITUD_MODIFICACION_PRES: "Datos de los Rubros de la Modificación",
    RETE_IVA: "Rete IVA",
    SALDO: "Saldo",
    SECUENCIA: "Secuencia",
    SEDE: "Sede",
    SEGUDIDAD_SOCIAL: "Seguridad Social",
    SEGUNADA: "Segunda",
    SEGURO: "Seguro",
    SELECCIONAR: "Seleccionar",
    SELECCIONAR_CATEGORIA: "Seleccione la Categoría",
    SELECCIONAR_CUENTA: "Seleccionar Cuenta",
    SELECCIONAR_TIPO: "Seleccione el Tipo",
    SELECCIONAR_DEPENDENCIA: "Seleccione la Dependencia",
    SELECCIONAR_DESCUENTOS: "Selecciona Impuestos y Descuentos",
    SELECCIONAR_IMPUESTOS: "Seleccionar Impuestos",
    SELECCIONAR_RUBRO: "Seleccionar Rubro",
    SELECCIONAR_VIGENCIA: "Seleccione una Vigencia",
    SELECCIONAR_CUENTA_CONTABLE_PADRE: "Seleccione la cuenta contable padre",
    SELECCIONAR_CUENTA_CONTABLE: "Seleccione la cuenta contable",
    SELECCIONAR_FORMA_PAGO:"Seleccione Forma de Pago",
    SELECCIONAR_CUENTAS_CONTABLES_CONCEPTO: "Seleccione las cuentas contables asociadas al concepto",
    SELECCIONAR_UNIDAD_EJECUTORA: "Debe seleccionar la unidad ejecutora",
    SELECCIONAR_CONCEPTO_INGRESO: "Debe seleccionar el concepto que afecta este ingreso",
    SELECCIONAR_ORDEN_DEV: "Debe seleccionar al menos una Orden de Devolución",
    SELECCIONAR_FORMA_INGRESO: "Debe seleccionar la forma de ingreso",
    SELECCIONAR_FECHA_INGRESO: "Debe seleccionar la fecha de consulta  de los ingresos",
    SELECCIONAR_FACULTAD: "Debe seleccionar una facultad",
    SELECCIONAR_SUCURSAL:"Seleccionar Sucursal",
    SELECCIONAR_TIPO_DOCUMENTO: "Seleccione el Tipo de Documento",
    SELECCIONAR_TIPO_CUENTA: "Seleccione el Tipo de Cuenta",
    SELECCIONAR_INVERSION:"Seleccionar Inversión",
    SELECCIONAR_CANCELACION_INVERSION:'Seleccionar Cancelación Inversión',
    SELECT_MES:"Seleccionar mes",
    SELECCIONAR_PROYECTO_CURRICULAR: "Debe seleccionar Proyecto Curricular",
    SELECCIONE: "Seleccione",
    SELECCIONE_AFECTACION: "Seleccione al menos un tipo de afectación",
    SELECCIONE_CRP: "Seleccione CRP",
    SELECCIONE_DEPENDENCIA: "Seleccione la Dependencia",
    SELECCIONE_FUENTE_FINANCIAMIENTO: "Seleccione la Fuente de Financiamiento",
    SELECCIONE_FUENTE_FINANCIAMIENTO_DESTINO: "Seleccione la Fuente de Financiamiento de Destino",
    SELECCIONE_FUENTE_FINANCIAMIENTO_ORIGEN: "Seleccione la Fuente de Financiamiento de Origen",
    SELECCIONE_PROVEEDOR: "Seleccionar Proveedor",
    SELECCIONE_RUBROS_FUENTE: "Seleccione los Rubros de la Fuente de Financiamiento",
    SELECCIONE_RUBRO_DESTINO: "Seleccione Rubro de Destino",
    SELECCIONE_RUBRO_ORIGEN: "Seleccione Rubro de Origen",
    SELECCIONE_TIPO_MODIFICACION: "Seleccione el tipo de Modificación",
    SELECCIONE_TIPO: "Seleccione el tipo",
    SELECCIONE_CARPETA_CONCEPTO: "Seleccione la carpeta en donde se ubicará el concepto",
    SELECCIONE_CUENTAS: "Seleccione al menos una Cuenta Contable",
    SELECCIONE_CUENTAS_HIJOS: "Únicamente puedes seleccionar cuentas que no tengan hijos",
    SELECCION_OPCION: "Selecciona una opción",
    SELECCION_RUBRO: "Seleccione el Rubro",
    SELECCION_CUENTAS:"Selección Cuentas Contables",
    SELECCION_UNIDAD_EJECUTORA: "Seleccionar Unidad Ejecutora",
    SIGLA: "Sigla",
    SOLICITUD: "Solicitud",
    SOLICITUD_AVANCE: "Solicitud de Avance",
    SOLICITUD_AVANCE_VALIDADA: "Esta solicitud ya fue validada",
    SOLICITUD_NECESIDAD: 'Solicitud Necesidad',
    SOLICITUD_CDP: "Solicitud de CDP No.",
    SOLICITUD_TRASLADO: "Solicitud de Traslado",
    SOLICITUD_RP: "Solicitud de Registro Presupuestal",
    SOLICITUDES_RP: "Solicitudes de Registro Presupuestal",
    SOLICITANTE:"Solicitante",
    SUCURSAL: "Sucursal",
    SUBCENTRO_COSTO: "Subcentro de costo",
    SUBTOTAL: "Subtotal",
    SUCURSALES: "Sucursales",
    S_AP001: "Asignación Inicial de Presupuesto aprobada.",
    S_542: "Se Actualizo exitosamente",
    S_5412: "Se canceló el compromiso ",
    S_543: "Se registró exitosamente",
    S_544: "Se aprobó el Ingreso ",
    S_545: "Se rechazó el Ingreso con el consecutivo ",
    S_554: "Se eliminó exitosamente",
    S_588: "El registro se actualizó exitosamente",
    S_900: "Se verificaron los requisitos exitosamente",
    S_A01: "Se Rechazo la Solicitud ",
    S_CDP001: "CDP Expedido",
    S_A12: "Operación para Anulación correctamente realizada para la solicitud",
    S_M001: "Indique una justificación por el rechazo",
    SM002: "Por favor indica una justificación!",
    S_MODP001: "Se registro la solicitud del Modificación presupuestal correctamente Con el Consecutivo No. ",
    S_MODP002: "Solicitud de Modificación Presupuestal Aprobada",
    S_MODP003: "CDP para la Modificación Presupuestal Correctamente Expedido",
    S_MODP004: "Solicitud De Modificación Presupuestal Anulada",
    S_OPP_01: "Registrada exitosamente",
    S_GIRO_01: "Giro registrado exitosamente con consecutivo No. ",
    S_GS001:"Sucursales desvinculadas",
    S_GS002:"Sucursales vinculadas",
    S_OP_ESTADO: "Estado de la orden de pago registrado",
    S_OPP_02: "Orden de Pago Actualizado Exitosamente con Consecutivo No. ",
    S_CHEQ_01: "Chequera registrada exitosamente con consecutivo No. ",
    S_SI1: "El registro del saldo inicial por un valor",
    S_SI2: "fue creado exitosamente en la cuenta contable",
    S_SI3: "para la cuenta bancaria",
    SMA_001: "Se realizó la solicitud de avance con consecutivo No. ",
    SOPORTE:"Soporte",
    SUCURSALES_REGISTRADAS: "Sucursales registradas",
    TABLA_INGRESOS_BANCO: "Lista de Consignaciones",
    TASA_NOMINAL: "Tasa Nominal",
    TASA:"Tasa",
    TELEFONO: "Teléfono",
    TERCERO: "Tercero",
    TIPO: "Tipo",
    TIPO_ACTIVIDAD:"Tipo de Actividad",
    TIPO_ANULACION: "Tipo de Anulación",
    TIPO_AVANCE: "Tipo de Avance",
    TIPO_CUENTA: "Tipo Cuenta",
    TIPO_COMPROBANTE: "Tipo de comprobante",
    TIPOS_COMPROBANTE: "Tipos de comprobantes",
    TIPO_COMPROMISO: "Tipo Compromiso",
    TIPO_CONTRATO:"Tipo Contrato",
    TIPO_DEPENDENCIA_SOLICITANTE: "Tipo Dependencia Solicitante",
    TIPO_DOCUMENTO: "Tipo Documento",
    TIPO_EMPLEADO: "Tipo Empleado",
    TIPO_FUENTE_FINANCIAMIENTO: "Tipo de Fuente de Financiamiento",
    TIPO_INGRESO: "Tipo de Ingreso",
    TIPO_MODIFICACION: "Tipo de Modificación Presupuestal",
    TIPO_MONEDA: "Tipo de moneda",
    TIPO_ORDEN_PAGO: "Tipo Orden Pago",
    TIPO_MOVIMIENTO: "Tipo Movimiento",
    TIPO_PERSONA: "Tipo Persona",
    TIPO_RECURSO:"Tipo Recurso",
    TIPOS_TRANSACCION:"Tipos Transacción",
    TIPO_NECESIDAD: "Tipo Necesidad",
    TIPO_ICA: "ICA",
    TIPO_IVA: "IVA",
    TIPO_RENTA: "ReteFuente",
    TIPO_RECIBO: "Tipo de Recibo",
    TITULO_APP: "GENERATOR-OAS",
    TITULO: "Título",
    TITULO_FUENTES_CONSULTAR: "Consultar Fuentes de Financiamiento",
    TITULO_FUENTES_CREAR: "Crear Fuentes de Financiamiento",
    TITULO_FUENTES_DETALLE: "Detalle Fuentes de Financiamiento",
    TITULO_SOLICITUD_CDP: "Solicitudes de Certificado de Disponibilidad Presupuestal",
    TITULOS:"Títulos",
    TRIBUTARIA: "Tributaria",
    TRASLADOS: "Traslados",
    TRM: " T.R.M",
    TRM_COMPRA: "TRM de la fecha de compra",
    TRM_COMPRA_DESCRIPCION: "** Tasa representativa de mercado del dia de la compra, para legalizaciones de gastos en moneda extranjera",
    TODOS: "Todos",
    TOTAL: "Total",
    TOTAL_ADICIONES: "Total Adiciones",
    TOTAL_ANULADO: "Total Anulado",
    TOTAL_COMPROMETIDO: "Total Comprometido",
    TOTAL_REDUCCIONES: "Total Reducciones",
    TOTAL_TRASLADOS: "Total Traslados",
    TRASLADO_FUENTE_FINANCIAMIENTO: "Traslado Fuente de Financiamiento",
    TIPO_TRANSACCION:"Tipo Transacción",
    TOTAL_DEVOLUCION_RUBRO:"El total de la devolución es igual a el distribuido en rubros",
    TOTAL_DEVOLUCION_RUBRO_ADVERTENCIA:"El total de la devolución no corresponde al distribuido en rubros",
    UNIDAD_EJECUTORA: "Unidad Ejecutora",
    VALIDANDO:"Validando",
    VALOR: "Valor",
    VALOR_ACTUAL:"Valor Actual",
    VALOR_ADICION: "Valor Adición",
    VALOR_AFECTACION: "Valor afectación",
    VALOR_CHEQUE_INICIAL_MAYOR:"El número de cheque inicial es menor que el final",
    VALOR_AFECTACION_FUNCIONAMIENTO: "Valor de la Afectación (Funcionamiento)",
    VALOR_APLICADO: "Valor Aplicado",
    VALOR_AVANCE: "Valor de Avance",
    VALOR_BASE: "Valor Base",
    VALOR_BASE_RETENCION: "Valor Base Retención",
    VALOR_BRUTO: "Valor Bruto",
    VALOR_CDP: "Valor CDP",
    VALOR_CONTRATACION: "Valor de la Contratación",
    VALOR_CRP: "Valor CRP",
    VALOR_DISPONIBLE: "Valor Disponible",
    VALOR_DISTRIBUCION: "Valor Distribución",
    VALOR_FUENTE: "Valor Fuente Financiamiento",
    VALOR_LEGALIZADO: "Valor Legalizado",
    VALOR_MAXIMO: "Valor Máximo",
    VALOR_NOMINAL_SALDO: "Valor Nominal Saldo",
    VALOR_NOM_SALDO_MON_NAL: "Valor Nominal Saldo para Moneda Nacional",
    VALOR_PAGO: "Valor Pago",
    VALOR_RUBRO: "Valor Rubro",
    VALOR_TOTAL: "Valor Total",
    VALOR_FACTURA_COMPRA: "Valor de la factura o compra sin IVA",
    VALOR_TRASLADO: "Valor Traslado",
    VALOR_SOLICITADO: "Valor Solicitado",
    VALOR_USD: "Valor en dólares",
    VALOR_DEVOLUCION:"Valor Devolución",
    VAL_IVA: "Valor IVA",
    VALOR_RECOMPRA:"Valor Recompra",
    VENCIMIENTO: "Vencimiento",
    VER_TRASLADO: "Ver translado",
    VER_TODOS: "Ver todos",
    VERSION:"Versión",
    VIGENCIA: "Vigencia",
    VINCULACION: "Vinculación",
    VOLVER_VIGENCIA_ACTUAL: "Volver a vigencia actual",
    APROPIACION_INICIAL: "Apropiación Inicial",
    INFORMACION_SALDO_APROPIACION: "Información Presupuestal de la Apropiación",
    INFORMACION_RUBROS_MODIFICACION_PRES: "Rubros Seleccionados",
    PLANEACION_APROBACION_INICIAL: "Planeación de la apropiación inicial",
    VER_ANULACIONES:"Ver Anulaciones",
    VER_FENECIDOS:"Ver Pasivos Fenecidos",
    VER_RESERVAS:"Ver Reservas",
    VER_SOLICITUD: "Ver Solicitud",
    VERSIONES: "Versiones",
    VALOR_NETO_GIRAR:"Valor Neto Girar",
    PROCESO: "Proceso",
    VAL_EJEC:"Ejecutado",
    VAL_PROY:"Proyectado",
    VENDEDOR:"Vendedor",
    BTN: {
        ACEPTAR: "Aceptar",
        ACTUALIZAR: "Actualizar",
        ADD_VALOR_BASE:"Agregar Valor Base",
        AGREGAR: "Agregar",
        AGREGAR_CODIGOS: "Agregar códigos",
        ANADIR: "Añadir",
        ANULAR:"Anular",
        APROBAR: "Aprobar",
        CREAR: "Crear",
        CANCELAR:"Cancelar",
        CREAR_TIPO_COMPROBANTE: "Crear tipo de comprobante",
        CONSULTAR_MOVIMIENTO: "Consultar movimiento asociado",
        CREAR_DEVOLUCION_TRIBUTARIA:"Crear Devolucion Tributaria",
        CREAR_ORDEN_DEVOLUCION:"Crear Orden Devolución",
        CREAR_RELACION_DEVOLUCION:"Crear Relación Devolución",
        CREAR_INGRESO:"Crear Ingreso",
        BORRAR: "Borrar",
        CERRAR:"Cerrar",
        CONFIGURAR: "Configurar",
        CONFIRMAR: "Confirmar",
        DESVINCULAR: "Desvincular",
        EDITAR: "Editar",
        GUARDAR: "Guardar",
        INACTIVAR: "Inactivar",
        LEGALIZAR: "Legalizar",
        LIMPIAR: "Limpiar",
        MOSTRAR_SUCURSALES: "Mostrar sucursales",
        NUEVO: "Nuevo",
        NUEVO_OP_PROVEEDOR: "Nuevo OP Proveedor",
        NUEVO_OP_NOMINA: "Nuevo OP De Nomina",
        NUEVO_OP_SEGURIDAD_SOCIAL: "Nuevo OP Seguridad Social",
        NUEVA_HOMOLOGACION: "Nueva Homologación",
        QUITAR: "Quitar",
        QUITAR_RUBRO: "Quitar",
        RECHAZAR: "Rechazar",
        REGISTRAR: "Registrar",
        REGRESAR: "Regresar",
        SELECCIONAR: "Seleccionar",
        SIGUIENTE: "Siguiente",
        SOLICITAR_RP: "Solicitar RP",
        SOLICITAR: "Solicitar",
        VER: "Ver",
        VER_RUBROS: "Ver rubros",
        VER_DETALLE: "Ver detalle",
        VER_SUCURSAL: "Ver sucursal",
        VERIFICAR: "Verificar Requisitos",
        VINCULAR: "Vincular",
        CONSULTAR: "Consultar",
        DESCARGAR_PDF: "Descargar PDF",
        VER_PAGOS:"Ver Pagos",
        VER_HOMOL:"Ver Homologación"
    },
    D_PLANES_CUENTA: {
        LISTADO_PLAN_CUENTAS: "Listado de Planes de Cuentas",
        NO_FILAS: "Número de Filas"
    },
    REPORTES_SPAGO: {
      REPORTES: "Reportes"
    },
    REPORTES: {
      LISTADO_APROPIACIONES: "Listado de Apropiaciones",
      FECHA_REPORTE: "Fecha del reporte",
      CDP: "Certificado de Disponibilidad Presupuestal",
      GENERAR_REPORTE: "Generar Reporte",
      RP: "Certificado de Registro Presupuestal"
    }
};

var text_en = {
    ACTUALIZAR: "Update",
    ACTUALIZADO_CORRECTAMENTE: "Correctly updated",
    ACTUALIZAR_CONCEPTO: "Update Concept",
    ACTA:"Record",
    ACTA_DEVOLUCION:"Devolution Act",
    AFECTACION: "Affectation",
    AFECTACION_CONCEPTO: "Enter Affection for Concept",
    AGREGAR_BANCO: "Add Bank",
    AGREGAR_CUENTA_BANCARIA: "Add Bank Account",
    AGREGAR_MAS_CUENTAS:"Do you wish to add another account?",
    AGREGAR_SUCURSAL: "Add Branch Office",
    APORTANTE:"Contributor",
    APROBACION_PRESUPUESTO: "Aprobación de Presupuesto",
    ALERTA_CAMPOS_BUSQUEDA: "Please, select at least one search field",
    ALERTA_COMPLETAR_DATOS: "Complete all fields",
    ALERTA_CREACION_COM_CORRECTA: "Support Document correctly created",
    ALERTA_CREACION_COM_INCORRECTA: "Error creating Support Document",
    ALERTA_CREACION_TIPOCOM_CORRECTA: "Support Document Type correctly created",
    ALERTA_CREACION_TIPOCOM_INCORRECTA: "Error creating Support Document Type",
    ALERTA_ERROR_RECHAZO: "Error recording Rejection",
    ALERTA_JUSTIFICACION_RECHAZO: "Point out Void reason",
    ALERTA_NO_REGISTROS: "There's no data for Support Document",
    ANALISIS_RIESGOS: "Risk Analysis",
    ARBOL_RUBROS_HOMMOLOGADO:"Arbol Rubros Homologados",
    ALERTA_RECHAZO_CORRECTO: "Rejection correctly recorded",
    ANULACION_RP: "RP Information Annulment",
    TOTAL_ANULADO_RP: "Total Rp Voided",
    BANCO_GIRADOR: "Issuer Bank",
    BANCO_RECEPTOR: "Receiver Bank",
    BENEFICIARIO: "Beneficiary",
    CAMPOS_OBLIGATORIOS:"You must complete highlight fields",
    CAMPOS_OBLIGATORIOS_SOLICITANTE:"You must complete applicant fields ",
    CAMPOS_OBLIGATORIOS_BENEFICIARIO:"You must complete beneficiary fields",
    CAMPOS_OBLIGATORIOS_AVANCE:"You must complete all fields to add Advance ",
    CARGANDO: "Loading",
    CAUSAL_REINTEGRO:"Refund Reason",
    CENTRO_COSTO: "Cost Center",
    CIERRE_PERIODO_PAC:"CLOSING",
    CHEQUERA:"Checker",
    CHEQUES_DISPONIBLES:"Available Checks",
    CHEQUE_INICIAL:"Initial cheque number",
    CHEQUE_FINAL:"Final cheque number",
    CIUDAD: "City",
    CLASE:"Kind",
    CLASIFICACION: "Classification",
    CODIGO: "Code",
    CODIGO_ACH: "ACH Code",
    CODIGO_BANCO: "Bank Code",
    CODIGO_PADRE: "Father Code",
    CODIGO_COMPROBANTE: "Support documento code",
    CONFIRMACION_ACTIVO_TIPO_COMPROBANTE: "Do you want to active the support document type",
    CONFIRMACION_INACTIVO_TIPO_COMPROBANTE: "Do you want to inactive the support document type",
    CONFIRMACION_EDICION_TIPO_COMPROBANTE: "Do you want to edit the support document type",
    COMPANIA: "Company",
    COMPROMISO: "Commitment",
    COMPRADOR:"Buyer",
    CONCEPTO: "Concept",
    CONCEPTOS: "Concepts",
    CONCEPTOS_SELECCIONADOS: "Selected Concepts",
    CONSULTA_APROBACION_INICIAL: "Consulta de Apropiación Inicial",
    CONSULTA_RELACIONES_DEVOL:"Look up Devolution Association",
    CONSULTA_INGRESOS_SIN_SF:"Income without finance situation",
    CONSULTA_RUBRO: "Consulta de Rubros",
    CONSULTA_RUBRO_HOMOLOGADO: "Approved Items Query",
    CONSULTA_INVERSIONES:"Investments Query",
    CONSULTA_CANCELACION_INVERSIONES:"Investments Cancelation Query",
    CONTROL_SALDOS_INICIALES:"Control Opening Balances Banks",
    CREACION_COMPROBANTE: "Support document creation",
    CREACION_CHEQUE: "Cheque Creation",
    CREACION_TIPO_COMPROBANTE: "Support document types creation",
    CREDITO: "Credit",
    CUENTA: "Account",
    CUENTAS_ASOCIADAS:"Related Account",
    CUENTAS_BANCARIAS: "Bank Accounts",
    CUENTAS_CONTABLES: "Accounting Accounts",
    CUENTA_CONTABLE: "Accounting Account",
    CUENTA_GIRO: "Money Order Account",
    CUENTA_RECEPTORA:"Receiver Account",
    DATOS_CANCELACION: "Cancellation Information",
    DATOS_BASICOS: "Basic Information",
    DE: "From",
    DEBITO: "Debit",
    DEFINITIVA: "Permanent",
    DEPARTAMENTO: "State",
    DESCRIPCION: "Description",
    DESCRIPCION_COMPROMISO: "Description Commitment",
    DESCRIPCION_PLAN: " Descripción del nuevo plan...",
    DESASOCIAR_CONCEPTO: "The concept and accounting accounts will be deleted",
    DETALLE_CRP: "Detail CRP",
    DETALLE_COMPROBANTE: "Support Document Detail",
    DETALLE_OP: "Detail Payment Orders",
    DETALLE_ORDEN_PAGO: "Detail Payment Orders",
    DETALLE_PROVEEDOR: "Detail Provider",
    DETALLE_UNIDAD_EJECUTORA: "Detail Executing Unit",
    DETALLE_TIPO_TRANSACCION: "Transaction Type Detail",
    DETALLE_VALOR_PAGO: "Detail Value Paid",
    DETALLE_VERSION:"Version Detail",
    DEVOLUCION:"Devolution",
    DEVOLUCION_TRIBUTARIA:"Tributary Devolution",
    DIRECCION: "Address",
    DISPONIBLE: "Disponible",
    DIAS_VENCIMIENTO:"Expiration days",
    EDITAR_CUENTA_BANCARIA: "Edit Bank Account",
    EDITAR_PLAN_CUENTAS: "Editar plan de Cuentas",
    EDITAR_SUCURSAL: "Edit Branch Office",
    EDICION_TIPO_COMPROBANTE: "Support Documento Edition",
    ELABORADO_POR: "Elaborated By",
    ENDOSAR_A_TERCERO: "Endorsement to third",
    ESTUDIO_MERCADO: "Estudio de Mercado",
    ESTUDIANTE_NO_ENCONTRADO:"Student not found",
    ENTIDAD: "Entity",
    ENTRADA_ALMACEN: "Warehouse Entryss",
    ESTADO: "State",
    ELEGIR: "Choose",
    E_A08:  "Need isn't approved",
    E_CI001:"Investment already has got a cancelation item",
    E_COMP1: "Choose category",
    E_COMP2: "Choose Agreement Type",
    E_ISF001: "Invalid Selected Item",
    E_LA0001:"Legalization Value Greater than advance payment",
    E_LT0001:"There is a version for the established dates",
    E_ISF002: "Error recording Income",
    E_OPGC_01:"Invalid pay order",
    E_RB003: "You must select a Rubro",
    E_GS007:"Error unlinking branches",
    E_GS008:"Error linking branches",
    ERROR_ACTUALIZAR: "Error updating",
    ERROR_ELIMINAR: "Deleting Error",
    ERROR_LLENAR: "Please fill all spaces",
    EMISOR:"Transmitter",
    EXTRAORDINARIO:"extraordinary",
    EXPIDIO: "Dispatched by",
    ESTUDIO_MERCADO: "Market Survey",
    FACTURA: "Bill",
    FECHA: "Date",
    FECHA_CANCELACION: "Cancellation Date",
    FECHA_COMPRA: "Buy Date",
    FECHA_CREACION: "Creation Date",
    FECHA_CIERRE:"Closing Date",
    FECHA_OFICIO: "Document Date",
    FECHA_REDENCION:"Redemption Date",
    FECHA_EMISION:"Release Date",
    FECHA_SOLICITUD: "Request Date:",
    FECHA_VENCIMIENTO: "Expiration Date",
    FILTRO_NOTIFICACION: "Filtrar notificación",
    FECHA_VENTA:"Sale Date",
    FECHA_PACTO:"Agreement Date",
    FINANCIACION: "Financing",
    FUENTE:"Source",
    FUENTES_AFECTADAS: "Affected Sources",
    FUENTES_FINANCIACION: "Sources of funding",
    FUENTE_FINANCIACION_CODIGO: "Code Sources of funding",
    GESTIONAR_CUENTAS_BANCARIAS: "Manage Bank Accounts",
    GESTIONAR_SUCURSALES: "Manage Branch Offices",
    IDENTIFICACION: "ID",
    INGRESE_NOMBRE_PLAN_CUENTAS: "Ingresa el nombre del plan de cuentas",
    INFORMACION_DINERO: "Values Information",
    INFORMACION_GENERAL: "General Information",
    INFORMACION_FECHAS : "Dates Information",
    INFORMACION_ADICIONAL_BANCO: "Bank Aditional Information",
    INFORMACION_DOCUMENTO_ING: "Income document information",
    INFORMACION_DEVOLUCION:"Devolution Information",
    INFORMACION_LEGALIZACIONES:"Legalizations Information",
    INFORMACION_PRESUPUESTAL: "Budget Information",
    INFORMACION_REG_CORRECTO: "Information correctly recorded",
    INFORMACION_TRASLADO: "Transfer Information",
    INFORMACION_TIPO_TRANSACCION:"Type Transaction Information",
    INFORMACION_REINTEGRO: "Refund Information ",
    INFORMACION_REGISTRO_REINTEGRO: "Information Refund Registration",
    INFORMACION_CHEQUERA: "Checker Information",
    INFORMACION_CHEQUE: "Cheque Information",
    INGRESOS_SIN_SF:"Income Without Finance Situation",
    INVERSIONES:"Investments",
    JUSTIFICACION_CONTRATO: "Contract Justification",
    LISTA_COMPROBANTES: "Supporting documents list",
    LISTA_TITULOS:"Certificates List",
    LISTA_ACTAS:"Acts List",
    LISTA_RAZON_DEVOLUCION:"Devolution Reason List",
    LISTA_RESERVAS: "Reservs List",
    LIQUIDACION: "Liquidation",
    MENSAJE_INICIAL: "Now get to start to develop",
    MES: "Month",
    MES_ABRIL: "April",
    MES_AGOSTO: "August",
    MES_DICIEMBRE: "December",
    MES_ENERO: "January",
    MES_FEBRERO: "February",
    MES_JULIO: "July",
    MES_JUNION: "June",
    MES_MARZO: "March",
    MES_MAYO: "May",
    MES_NOBIEMBRE: "November",
    MES_OCTUBRE: "October",
    MES_SEPTIEMBRE: "september",
    SELECT_MES:"Select Month",
    MARCO_LEGAL: "Legal Framework",
    MODO_PAGO: "Payment Method",
    MODALIDAD_DE_SELECCION: "Selection mefinancierothod",
    MODIFICACION_FUENTES_FINANCIAMIENTO: "Modify Source of Funding",
    MOVIMIENTOS_APROPIACION: "Movimientos De la Apropiación",
    MSN_DEBE_ASOCIAR_CUENTAS: "You must asociate account information to devolution",
    MSN_DEBE_CONCEPTO: "You must select at least one Concept",
    MSN_DEBE_ENTRADA_ALMACEN: "Should select warehouse entry",
    MSN_DEBE_IVA: "You must indicate the value of the IVA in the Payment Value Section",
    MSN_DEBE_LIQUIDACION: "You must select a liquidation",
    MSN_DEBE_MIN_CONCEPTO: "Must affect at least one concept",
    MSN_DEBE_PROVEEDOR: "You must select Supplier for the Payment Order",
    MSN_DEBE_CHEQUERA:"You should select a checker",
    MSN_DEBE_REGISTRO: "You must select Certificate Registry Budget",
    MSN_DEBE_TIPO_OP: "You must select el tipo de Documento en la in Payment Amount Section",
    MSN_DEBE_TERCERO: "You must select Third",
    MSN_DEBE_TOTAL_AFECTACION: "The total value of the assignment is different from the value of the payment order",
    MSN_DEBE_UNIDAD: "You must select Executing Unit",
    MSN_DEBE_VAL_BASE: "You must indicate the Ordinary Value in the Payment Value Section",
    MSN_SUPERA_SALDO: "exceeds the value of the balance",
    MSN_SOL_NO_ENC: "Applicant not found",
    MSN_BEN_NO_ENC: "Beneficiary not found",
    MSN_RESP_NO_ENC: "Manager not found",
    MSN_TOTAL_AECTACION: "The total of the affectation to the Concepts of the Rubro",
    MSN_TIPO_AVANCE: "Select at least one Advance",
    MSN_Y: "and",
    NATURALEZA: "Nature",
    NOMBRE: "Name",
    NOMBRE_COMPROBANTE: "Support Document Name",
    NECESIDAD_FECHA_RECHAZO: "Rejection date",
    NECESIDAD_JUSTIFICACION_RECHAZO: "Rejection justification",
    NECESIDAD_JUSTIFICACION_DESCRIPCION_RECHAZO: "Rejection Description",
    NOMINA: "Salary",
    NOTIFICACION_PENDIENTE: "Notificaciones Pendientes",
    NOTIFICACION_VISTA: "Notificaciones Vistas",
    NO_APLICA: "No data",
    NO_CDP: "No. CDP",
    NO_CRP: "No. CRP",
    NO_CHEQUE:"Cheque Number",
    NUM_CHEQUES:"Number of checks",
    NUMERO: "Number",
    NO_DATA: "No available data",
    NO_ENCONTRADO: "Not found",
    NO_DOCUMENTO: "No. Document",
    NO_ITEMS: "No. Items",
    NO_REQUISITOS_REGISTRADOS: "No data recorded for this Advance",
    NUMERO_ELABORACION: "Elaboration Number",
    NO_QUINCENA: "Nro. Fortnight",
    NOMINAL:"Nominal",
    NUEVA_CUENTA: "Nueva Cuenta",
    NUEVA_CATEGORIA: "Nueva Categoria",
    NUEVO_OP_PROVEEDOR: "New PO Supplier",
    NUEVO_PLAN_CUENTAS: "Crear Nuevo Plan de cuentas",
    NUEVO_COMPROMISO: "Nuevo Compromiso",
    NUEVO_TIPO_COMPROMISO: "Nuevo Tipo Compromiso",
    NUMERO_CUENTA:"Account number",
    NUMERO_OPERACION:"Operation Number",
    NUMERO_TRASLADO: "Transfer Number",
    OPCIONES: "Options",
    OPERACION: "Operation",
    OPERAR_CONCEPTO: "Operate Concept",
    ORDEN_PAGO: "Payment Orders",
    ORDINARIO:"Ordinary",
    ORDEN_PAGO_ACTUALIZAR: "Update Payment Orders",
    ORDEN_PAGO_SELECCIOANR: "Select Payment Order",
    ORDEN_DEVOLUCION: "Devolution Order",
    PERIODO:"Time",
    PACTO_VENTA: "Sales Agreement",
    PAIS: "Country",
    PLANES_CUENTAS: "Planes de Cuentas",
    PAGOS_REALIZADOS: "Payment Information",
    PAGO_MATRICULA: "Registration Payment",
    PAGO_SEGURO: "Insurance Payment",
    PAGO_CARNET: "ID Card Payment",
    PLANTA: "Floor",
    PLAN_CUENTAS_MAESTRO: "Plan de Cuentas Maestro",
    POR_IVA: "% IVA",
    PRIMERA: "Frist",
    PROVEEDOR: "Provider",
    RAZON_DEVOLUCION:"Devolution Reason",
    RECIBO: "Receipt",
    REDONDEO: "Round",
    REFERENCIA_PAGO: "Payment Reference",
    REGISTRO_PRESUPUESTAL: "Certificate Registry Budget",
    REGISTRO_TITULO:"Record Certificates",
    REGISTRO_RUBRO: "Registro de Rubro",
    REINTEGROS:"Refunds",
    REPORTE_PAC:"REPORT INCOME AND OUTCOME",
    REPORTE_PAC_TOTALES:"REPORT INCOME AND OUTCOME SUM BY ENTRY",
    RESPONSABLE: "Responsible",
    RELACION_DEVOLUCION: "Devolution Association",
    RUBRO: "Rubro",
    S_SI1: "The Initial Balance value ",
    S_SI2: "was succesfully created to Accounting Account",
    S_SI3: "to Bank Account",
    SALDO: "Balance",
    S_GS001:"De-linked branches",
    S_GS002:"Linked branches",
    S_CHEQ_01: "Checkbook registered successfully with consecutive No. ",
    SOLICITANTE:"Applicant",
    SECUENCIA: "Sequence",
    SEDE: "Headquarters",
    SEGUNADA: "Second",
    SELECCIONAR: "Select",
    SELECCIONE_AFECTACION: "Select at least one afectación",
    SELECCIONAR_CUENTA: "Select Account",
    SELECCIONAR_RUBRO: "Select Item",
    SELECCIONAR_IMPUESTOS: "Select taxes",
    SELECCIONE_CRP: "Select CRP",
    SELECCIONE_CUENTAS: "Select at least one Accounting Account",
    SELECCIONE_CUENTAS_HIJOS: "You must select accounts without children",
    SELECCIONE_PROVEEDOR: "Select Provider",
    SELECCION_UNIDAD_EJECUTORA: "Select Executing Unit",
    SELECCIONAR_TIPO_DOCUMENTO: "Select Document Type",
    SELECCIONAR_TIPO_CUENTA: "Select Account Type",
    SELECCIONAR_FORMA_PAGO:"Select Payment Method",
    SELECCIONAR_ORDEN_DEV: "You must select at least one Return Order",
    SELECCIONAR_SUCURSAL:"Select Branch Office",
    SELECCIONAR_INVERSION:"Choose Investments",
    SELECCIONAR_CANCELACION_INVERSION:"Choose Cancelation Investments",
    SELECCION_CUENTAS:"Select Accounting Account",
    SOLICITUD_TRASLADO: "Transfer Request",
    SOLICITUD_RP: "RP Request",
    SOLICITUDES_RP: "RP Requests",
    SOPORTE:"Support",
    SUBCENTRO_COSTO: "Cost subcenter",
    SUCURSAL: "Branch Office",
    SUCURSALES_REGISTRADAS: "Registered Branch Offices",
    TELEFONO: "Phone",
    TASA:"Rate",
    TASA_NOMINAL: "Nominal Rate",
    TIPO: "Type",
    TIPO_ACTIVIDAD:"Activity Type",
    TIPO_CUENTA: "Account Type",
    TIPO_COMPROBANTE: "Supporting document type",
    TIPOS_COMPROBANTE: "Supporting document types",
    TIPO_CONTRATO:"Contract Type",
    TIPO_DOCUMENTO: "Document Type",
    TIPO_EMPLEADO: "Employee Type",
    TIPO_MONEDA: "Currency",
    TIPO_PERSONA: "Type Person",
    TIPO_RECURSO:"Asset Type",
    TIPO_RECIBO: "Receipt Type",
    TIPO_TRANSACCION:"Transaction Type",
    TIPOS_TRANSACCION:"Transaction Types",
    TITULO: "GENERATOR-OAS",
    TITULOS:"certificates",
    TOTAL_DEVOLUCION_RUBRO:"The whole return is equal to the one distributed in items",
    TOTAL_DEVOLUCION_RUBRO_ADVERTENCIA:"The whole return ist'n equal to the one distributed in items",
    TRASLADOS: "Transfers",
    TRIBUTARIA: "Tax",
    TRM: " Official Exchange Rate",
    UNIDAD_EJECUTORA: "Executing Unit",
    VALIDANDO:"Validating",
    VALOR: "Value",
    VALOR_BASE_RETENCION: "Base Value Withholding",
    VALOR_DEVOLUCION:"Devolution Value",
    VALOR_BRUTO: "Gross Value",
    VALOR_CHEQUE_INICIAL_MAYOR:"Initial cheuqe number greater than final cheque number",
    VALOR_LEGALIZADO: "Legalized Value",
    VALOR_CRP: "Value CRP",
    VALOR_MAXIMO: "Maximum Value",
    VALOR_NOMINAL_SALDO: "Balance Nominal Value",
    VALOR_NOM_SALDO_MON_NAL: "Balance Nominal Value for National Currency",
    VALOR_PAGO: "Amount Paid",
    VALOR_SOLICITADO: "Requested Value",
    VALOR_USD: "USD Value",
    VAL_EJEC:"Executed",
    VAL_PROY:"Planned",
    VAL_IVA: "Value IVA",
    VALOR_ACTUAL:"Actual Value",
    VALOR_NETO_GIRAR:"Net Value to Pay",
    VER_TRASLADO: "See Transfer",
    VER_TODOS: "See all",
    VERSIONES: "Versions",
    VERSION:"Version",
    VIGENCIA: "Validity",
    VINCULACION: "Bonding",
    VOLVER_VIGENCIA_ACTUAL: "Back to current Term",
    D_PLANES_CUENTA: {
        LISTADO_PLAN_CUENTAS: "Listado de Planes de Cuentas",
        NO_FILAS: "Número de Filas"
    },
    BTN: {
        ADD_VALOR_BASE:"Add Base Value Withholding",
        ACEPTAR: "Accept",
        AGREGAR: "Add",
        AGREGAR_CODIGOS: "Add codes",
        ANULAR: "Void",
        APROBAR: "Aprobar",
        CANCELAR: "Cancel",
        CONFIRMAR: "Confirmar",
        CREAR_TIPO_COMPROBANTE: "Create Support Document type",
        CREAR_DEVOLUCION_TRIBUTARIA:"Create Tributary Devolution",
        CREAR_ORDEN_DEVOLUCION:"Create Devolution Order",
        CREAR_RELACION_DEVOLUCION:"Create Devolution Relation",
        CREAR_INGRESO:"Create Income",
        CERRAR:"Close",
        CONSULTAR_MOVIMIENTO: "See asociated activity",
        DESVINCULAR: "Disassociate",
        EDITAR: "Edit",
        INACTIVAR: "Inactive",
        LEGALIZAR: "Legalizar",
        MOSTRAR_SUCURSALES: "See Branch Offices",
        NUEVO: "New",
        QUITAR_RUBRO: "Quitar",
        REGRESAR: "Previous",
        REGISTRAR: "Registrar",
        SELECCIONAR: "Select",
        SIGUIENTE: "Next",
        SOLICITAR_RP: "Solicitar RP",
        VER: "See",
        VER_RUBROS: "See Rubros",
        VER_DETALLE: "See detail",
        VER_SUCURSAL: "See Branch Office",
        VINCULAR: "Associate",
        CONSULTAR: "Consult",
        DESCARGAR_PDF: "PDF Download",
        VER_PAGOS:"Look Payment",
        VER_HOMOL:"Show Homologation"
    },
    REPORTES: {
      LISTADO_APROPIACIONES: "Apropiations List",
      FECHA_REPORTE: "Reporte date",
      CDP: "Certificado de Disponibilidad Presupuestal",
      GENERAR_REPORTE: "Make Report",
      RP: "Certificado de Registro Presupuestal"
    }
};

//
function ordenar(p_objeto) {
    var keys = [],
        k, i, len;
    for (k in p_objeto) {
        if (p_objeto.hasOwnProperty(k)) {
            keys.push(k);
        }
    }
    keys.sort();
    len = keys.length;

    for (i = 0; i < len; i++) {
        k = keys[i];
        console.log(k + ':' + ' "' + p_objeto[k] + '",');
    }
}

function comparar(p_objeto1, p_objeto2) {
    for (var key in p_objeto1) {

        if (p_objeto1.hasOwnProperty(key)) {
            console.log(key);
            //correr dos
            for (var key_2 in p_objeto2) {
                if (p_objeto2.hasOwnProperty(key_2) && key_2 == key) {
                    console.log("      Esta");
                }
            }
            // --
        }
    }
}
//
angular.module('financieraClienteApp')
    .config(function($translateProvider) {
        $translateProvider
            .translations("es", text_es)
            .translations("en", text_en);
        $translateProvider.preferredLanguage("es");
        $translateProvider.useSanitizeValueStrategy("sanitizeParameters");
    });

//ordenar(text_en)
//comparar(text_es.BTN, text_en.BTN)
